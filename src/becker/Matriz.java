...
public class Matriz {
    ...
    public void laberinto(){
        this.posicions = new int[30];
        this.posiciona = new int[30];
        this.robot= new Robot(ciudad, 11,1, Direction.EAST);
        int fila=10;
        int columna=0;
        int i=0;
        int k=0;
        while (robot.getAvenue()!=14){   
            int j=0;
            while(j<2){
                while (robot.frontIsClear()==true){
                    ...
                    if (robot.canPickThing()){
                        robot.pickThing();
                        posicions[k] = robot.getStreet();
                        posiciona[k] = robot.getAvenue();
                        k++;
                    }
                }
                robot.turnLeft();
            ...
            while (fila!=robot.getStreet()){
                ...
                if(robot.getAvenue()==14){
                    fila=robot.getStreet();
                }
            }
            robot.turnLeft();
            fila--;
    ...
    public void Posiciones(){
        int p=0;
        for(p=0;p<posiciona.length;p++){
            System.out.println("Street: " +posicions[p] +"  Avenue: "+posiciona[p]);
        }
    }
}